...
final class NStockController extends CoreController
{
    ...
    public static function addnew(Request $req, Response $res, array $args){
      try{
        $jsonData['features'][] = $req;
        $newJsonString = json_encode($jsonData, JSON_PRETTY_PRINT);
    
        file_put_contents('localizacoes.json', $newJsonString);
    
        $response->getBody()->write('Localização adicionada com sucesso!');
        return $response;
